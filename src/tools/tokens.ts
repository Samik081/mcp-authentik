...
export function registerTokenTools(
  ...
  registerTool(server, config, {
    ...
    title: "List Tokens",
    description:
      "List tokens with optional filters for identifier, intent, managed status, and search.",
    ...
    inputSchema: {
      identifier: z
        .string()
        .optional()
        .describe("Filter by exact token identifier"),
      intent: z
        .enum(["api", "app_password", "recovery", "verification"])
        .optional()
        .describe("Filter by token intent"),
  ...
  registerTool(server, config, {
    ...
    title: "Create Token",
    description:
      "Create a new token with an identifier, optional intent, description, and expiration settings.",
    accessTier: "full",
    annotations: {
      readOnlyHint: false,
      destructiveHint: false,
    ...
    inputSchema: {
      ...
      user: z
        .number()
        .optional()
        .describe("User ID to associate the token with"),
    },
    handler: async (args) => {
  ...
  registerTool(server, config, {
    ...
    title: "Update Token",
    description:
      "Update an existing token. Only provided fields are modified (partial update).",
    accessTier: "full",
    annotations: {
      readOnlyHint: false,
  ...
  registerTool(server, config, {
    ...
    title: "Delete Token",
    description:
      "Delete a token by its identifier. This action is irreversible.",
    accessTier: "full",
    annotations: {
      readOnlyHint: false,
      destructiveHint: true,
  ...
  registerTool(server, config, {
    ...
    description:
      "View the raw key value of a token. This is a privileged operation that is logged.",
    accessTier: "full",
    annotations: {
      readOnlyHint: true,
      destructiveHint: false,
  ...
  registerTool(server, config, {
    ...
    title: "Set Token Key",
    description:
      "Set a custom key value for a token. Requires authentik_core.set_token_key permission.",
    accessTier: "full",
    annotations: {
      readOnlyHint: false,
      destructiveHint: true,
