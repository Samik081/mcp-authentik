...
export function registerRacTools(
  ...
  registerTool(server, config, {
    ...
    description:
      "List RAC (Remote Access Control) endpoints with optional filters.",
  ...
  registerTool(server, config, {
    ...
    inputSchema: {
      ...
      protocol: z
        .enum(["rdp", "vnc", "ssh"])
        .describe("Connection protocol (required)"),
      ...
      property_mappings: z
        .array(z.string())
        .optional()
        .describe("List of property mapping UUIDs"),
      maximum_connections: z
        .number()
        .optional()
  ...
  registerTool(server, config, {
    ...
    description:
      "Update an existing RAC endpoint. Only provided fields are modified (partial update).",
    accessTier: "full",
    annotations: {
      readOnlyHint: false,
    ...
    inputSchema: {
      ...
      protocol: z
        .enum(["rdp", "vnc", "ssh"])
        .optional()
        .describe("New connection protocol"),
      ...
      property_mappings: z
        .array(z.string())
        .optional()
        .describe("New list of property mapping UUIDs"),
      maximum_connections: z
        .number()
        .optional()
        .describe("New maximum concurrent connections"),
    },
    handler: async (args) => {
  ...
  registerTool(server, config, {
    ...
    description:
      "Delete a RAC endpoint by its UUID. This action is irreversible.",
    accessTier: "full",
    annotations: {
      readOnlyHint: false,
      destructiveHint: true,
  ...
  registerTool(server, config, {
    ...
    description:
      "List RAC connection tokens with optional filters. Tokens are system-managed (no create).",
    accessTier: "read-only",
    annotations: {
      readOnlyHint: true,
      destructiveHint: false,
  ...
  registerTool(server, config, {
    ...
    description:
      "Delete a RAC connection token by its UUID. This action is irreversible.",
    accessTier: "full",
    annotations: {
      readOnlyHint: false,
      destructiveHint: true,
    ...
    category: "rac",
    inputSchema: {
      connection_token_uuid: z
        .string()
        .describe("Connection token UUID to delete"),
    },
    handler: async (args) => {
