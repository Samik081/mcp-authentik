...
describe('handler: authentik_users_list', () => {
  ...
  it('returns JSON text of user list on success', async () => {
    const fakeUsers = { pagination: { count: 1 }, results: [{ pk: 1, username: 'admin' }] };
    (mockClient.coreApi.coreUsersList as ReturnType<typeof vi.fn>).mockResolvedValueOnce(fakeUsers);
  ...
  it('returns isError when client throws', async () => {
    (mockClient.coreApi.coreUsersList as ReturnType<typeof vi.fn>).mockRejectedValueOnce(
...
describe('handler: authentik_users_get', () => {
  ...
  it('calls coreApi.coreUsersRetrieve with correct id', async () => {
    ...
    (mockClient.coreApi.coreUsersRetrieve as ReturnType<typeof vi.fn>).mockResolvedValueOnce(fakeUser);
    ...
    expect(mockClient.coreApi.coreUsersRetrieve).toHaveBeenCalledWith({ id: 42 });
    expect(result.isError).toBeFalsy();
  });
...
describe('handler: authentik_users_delete (full tier)', () => {
  ...
  it('calls coreApi.coreUsersDestroy on success', async () => {
    ...
    (mockClient.coreApi.coreUsersDestroy as ReturnType<typeof vi.fn>).mockResolvedValueOnce(undefined);
    const server = createServer();
    registerAllTools(server, mockClient, makeConfig());
    const { client, cleanup } = await connectTestClient(server);
    ...
    expect(result.isError).toBeFalsy();
    expect(mockClient.coreApi.coreUsersDestroy).toHaveBeenCalledWith({ id: 42 });
    await cleanup();
  });
});
